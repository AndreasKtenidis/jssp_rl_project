...
def run_episode(env, gnn, actor, critic, edge_index, edge_weights, epsilon=0.1, device=None):
    ...
    while not done:
        x = prepare_features(env, edge_index, device=device)  # already handles device
        node_embeddings = gnn(x, edge_index, edge_weights)

        valid_actions, top_k_mask = select_top_k_actions(env, device=device)  # ensure mask is on same device

        probs, _ = actor(node_embeddings, env, mask=top_k_mask)  
        value = critic(node_embeddings, env)

        if torch.rand(1).item() < epsilon:
            action = valid_actions[torch.randint(len(valid_actions), (1,), device=device)].item()
        else:
        ...
        values.append(value)
        rewards.append(torch.tensor(reward, dtype=torch.float32, device=device))  # ensure reward is on GPU
