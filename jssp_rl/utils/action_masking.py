...
def select_top_k_actions(env, top_k=5):
    num_ops = env.num_jobs * env.num_machines
    valid_mask = torch.zeros(num_ops, dtype=torch.bool)
    priorities = []
    ...
    if len(priorities) == 0:
        top_k_mask = torch.zeros(num_ops, dtype=torch.bool)
        return torch.tensor([], dtype=torch.long), top_k_mask
    ...
    return torch.tensor(selected_indices, dtype=torch.long), top_k_mask
