...
from config import (
    lr, num_epochs, batch_size, VAL_LIMIT, BEST_OF_K, PROGRESS_EVERY,
    meta_iterations, meta_batch_size, inner_steps, inner_lr,
    meta_lr, inner_update_batch_size, validate_every, est_beta, use_est_boost
)
...
# === Plot Convergence ===
plot_rl_convergence(
    [entry["val_stoch_best_of_k"] for entry in log_data],
    save_path=os.path.join(plot_dir, "rl_convergence.png"),
)
...
# rollout με greedy policy
stats = rollout_once(env, actor_critic, device, use_est_boost = use_est_boost,   
        est_beta = est_beta, mode="greedy")

plot_gantt_chart(
    env.extract_job_assignments(),
    save_path=os.path.join(gantt_dir, "gantt_chart.png"),
) 
